...
export function BusinessDesign({ page, technicalNotes = false, roles = [] }: { page: Page; technicalNotes?: boolean; roles?: readonly string[] }) {
  ...
  if (page.id === 'propuestas') return <><PropuestasScreen />{technicalNotes && <p className="technical-note">GET /v1/consultas/propuestas_pendientes, POST /v1/comandos/confirmar_documento y POST /v1/comandos/rechazar_propuesta implementados y con adaptador real (`realPropuestasAccess`); activación detrás de `featureFlags.pendingProposalsIntegration` (hoy `false`). Primer feature con comandos de escritura: usa `createCommandIntent` (`src/api/idempotency.ts`) para una `Idempotency-Key` estable por acción. La descarga de evidencia (G-03) sigue fuera de alcance — la decisión se toma solo con los datos declarados.</p>}</>;
