...
def _matrices_y_requisitos(session: Session, tenant_id: str, oc: dict[str, Any], desde: date, hasta: date) -> tuple[list[dict[str, Any]], bool]:
    ...
    requisitos_particulares = [_requisito(dict(f), particular=True) for f in particulares]
...
def _evaluar_oc(session: Session, tenant_id: str, oc: dict[str, Any], legajos: list[dict[str, Any]],
    ...
    tramos, sin_matriz = _matrices_y_requisitos(session, tenant_id, oc, inicio, fin)
        ...
        for tramo in tramos:
            for req in tramo["requisitos"]:
                ...
                resultado = evaluar_requisito_documental(EvaluacionDocumentalEntrada(
                    ...
                    tuple(evidencias.get((legajo["sujeto_id"], req.requisito_definicion_id), ())),
                ))
                resultados.append({
    ...
    for legajo in detalle_legajos:
        legajo.pop("_resumen")
    return {"estado": resumen_oc, "tramos": tramos, "legajos": detalle_legajos}
